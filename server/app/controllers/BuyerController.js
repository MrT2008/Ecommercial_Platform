...
class BuyerController {
  ...
  viewAllPaymentMethod = async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "List of payment method of user:", paymentMethodList });
    } catch (error) {
      console.log(error);
      res.status(500).json({ message: "Internal Server Error" });
    }
    ...
    try {
      ...
      if (existingPaymentMethod) {
        if (existingPaymentMethod.isDeleted) {
          ...
          return res
            .status(200)
            .json({ message: "Create payment method sucessfully", existingPaymentMethod });
        } else {
      ...
      return res
        .status(200)
        .json({ message: "Create payment method sucessfully", newPaymentMethod });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal Server Error" });
    }
  ...
  removePaymentMethod = async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Payment method removed sucessfully", paymentMethodToBeDeleted });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal Server Error" });
    }
  ...
  setDefaultPaymentMethod = async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Sucessfully change default payment method", updatedPaymentMethod });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal Server Error" });
    }
  ...
  removeProductFromCart = async (req, res) => {
    try {
      ...
      return res
        .status(200)
        .json({ message: "Product removed from cart sucessfully", productToBeRemoved });
    } catch (error) {
      console.error(error);
      res.status(500).json({ message: "Internal Server Error" });
    }
  ...
  proceedWithCheckout = async (req, res) => {
    ...
    try {
      const { buyerId } = req.params;
      const { paymentMethod, productIds } = req.body;
      let payment = {};
  ...
  viewAllOrderByStatus = async (req, res) => {
    try {
      ...
      for (const ordersItem of ordersList) {
        const ordersDetail = await models.OrderDetail.findAll({
          where: { orderId: ordersItem.id },
        });
        const productList = [];
  ...
  getAllShippingInfo = async (req, res) => {
    try {
      ...
      const resMessage = !userShippingInfo
        ? "User have no shipping information"
        : "Sucessfully retrieve all user shipping information";
  ...
  setDefaultShippingInformation = async (req, res) => {
    try {
      ...
      return res.status(200).json({
        message: "Sucessfully change default shipping information",
